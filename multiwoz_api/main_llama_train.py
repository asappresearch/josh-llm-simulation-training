...
set_to_run = sorted(train_ids)[:200]
...
for idx, convo_key in enumerate(tqdm(set_to_run)):
    ...
    while len(messages)<15:
        ...
        if abs(current_percent_correct-result)>0.01:
            current_percent_correct = result
            training_data.append((copy.deepcopy(agent.messages_full), True))
        else:
            training_data.append((copy.deepcopy(agent.messages_full), False))
    ...
    with open(f"{cwd}/records/test_training_data/simulation_{convo_key.split('.json')[0]}_{time.ctime().replace(' ','_')}.json", 'w') as file:
        json.dump({'result':result, 'failed_apis':failed_api, 'clean_messages':messages, 'training_data': training_data, 'internal_messages':agent.messages_full, 'id':convo_key}, file, indent=2) # , 'internal_messages':agent.messages_full
print(results)
print(failed_api)
