...
from multiwoz_api.agent_simulator_mistral_v2 import AgentSimulator
...
if use_peft:
    ...
    #model = PeftModel.from_pretrained(model, '/root/git/multiwoz-api/checkpoints/kto-works-lambdaD=1.7_checkpoint-240')
    model = PeftModel.from_pretrained(model, '/root/multiwoz-api/checkpoints/kto-works')
    #model = PeftModel.from_pretrained(model, '/root/multiwoz-api/checkpoints/llama8instructsft-checkpoint-78')
...
for idx, convo_key in enumerate(tqdm(set_to_run)):
    ...
    user_llm = 'gpt-4o'
    alt_llm = False
    if alt_llm: user_llm = 'gpt-4-turbo'
    user = UserSimulator(goals=conversation_state.goals, modelname=user_llm)
    ...
    while len(messages)<15:
        ...
        user_response, cost = user.step(messages, client)
    ...
    with open(f"{cwd}/records/vanilla_v3_test_Aug24_NoPeft_NoMergeUnload/simulation_{convo_key.split('.json')[0]}_{time.ctime().replace(' ','_')}.json", 'w') as file:
