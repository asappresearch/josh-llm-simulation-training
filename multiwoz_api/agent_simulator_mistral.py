...
class AgentSimulator:
    ...
    def step(self, messages, conversation_state):
        ...
        messages_as = copy.deepcopy(self.messages_full)
        count=0
        command = None
        while count < 3:
            ...
            for command_type, command in parsed:
                ...
                if command_type=='THOUGHTS':
                    ...
                    # self.messages_full.append({'role':'assistant', 'content':'THOUGHTS '+command})
                elif command_type == 'SPEAK':
                    self.messages_full.append({'role':'assistant', 'content':thought_string+'SPEAK '+command+' <COMMAND_END>'})
                    messages_as.append({'role':'assistant', 'content':command})
                    return [{'role':'assistant', 'content':command}], messages_as
                    ...
                    # Add the return
                    messages_as.append({'role':'assistant', 'content':json.dumps(output)})
                    self.messages_full.append({'role':'assistant', 'content':'APIRETURN '+json.dumps(output)})
                else:
                    self.messages_full.append({'role':'assistant', 'content':'ERROR: INVALID COMMAND TYPE'})
            count+=1
        if not command:
            command = ''
            self.messages_full.append({'role':'assistant', 'content':command})
            messages_as.append({'role':'assistant', 'content':command})
        return [{'role':'assistant', 'content':command}], messages_as
