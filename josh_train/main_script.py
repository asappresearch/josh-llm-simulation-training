...
from user_simulator_script import UserSimulator
...
for idx, convo_key in enumerate(tqdm(set_to_run)):
    ...
    with open(f"{cwd}/records/official_openai_runs/val_run_script_0/simulation_{convo_key.split('.json')[0]}_{idx}_{time.ctime().replace(' ','_')}.json", 'w') as file:
