...
for idx, convo_key in enumerate(tqdm(set_to_run)):
    ...
    agent = AgentSimulator(api_examples, valid_api_defs, client=client, modelname='ft:gpt-4o-mini-2024-07-18:josh-llm-0807:9thBU9v9')# AgentSimulator(api_examples, valid_api_defs, model=model, tokenizer=tokenizer)
    ...
    with open(f"{cwd}/records/ft_mini/simulation_{convo_key.split('.json')[0]}_{time.ctime().replace(' ','_')}.json", 'w') as file:
